package com.stayrascal.spark.kafka.SurvivalRegression
...
/**
  * 加速失效时间模型(AFT: Accelerated Failure Time),用于检查数据参数生成回归模型
  * 它描述了生存时间对数的模型，通常被称为生存分析的对数线性模型
  * 不用于为相同目的设计的比例风险模型，AFT模型更容易并行化，
  *
  * Evaluator:
  * - RegressionEvaluator => 回归问题
  * - BinaryClassificationEvaluator => 二进制数据
  * - MulticlassClassificationEvaluator => 多类问题
  *
  */
object SurvivalRegressionApp {
  def main(args: Array[String]): Unit = {
    val spark: SparkSession = SparkSession.builder().appName("Survival Regression Example").master("local[*]").getOrCreate()

    val data: Dataset[Row] = loadData(spark)
    ...
    val paramGrid = new ParamGridBuilder()
      .addGrid(AFT.maxIter, Array(100, 500, 1000))
      .addGrid(AFT.tol, Array(1E-2, 1e-6))
      .build()

    val RegEvaluator = new RegressionEvaluator()
      .setLabelCol(AFT.getLabelCol)
      .setPredictionCol(AFT.getPredictionCol)
      .setMetricName("rmse")
    ...
    println(cvModel.avgMetrics) //参数对应的平均度量
    println(cvModel.getEstimatorParamMaps.length)
    println(cvModel.getEstimatorParamMaps) // 参数组合的集合

    cvModel.getEstimator.extractParamMap() // 评估的参数

    cvModel.getEvaluator.isLargerBetter // 评估的度量值是大的好，还是小的好
    cvModel.getNumFolds //交叉验证的折数

    val predictDF: DataFrame = cvModel.transform(testDF).selectExpr("label", "censor", "round(prediction, 2) as prediction").orderBy("label")
    predictDF.show
