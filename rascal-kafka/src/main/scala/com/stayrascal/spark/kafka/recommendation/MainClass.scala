...
object MainClass {
  ...
  def evaluatePrecision(testingSet: RDD[Rating], userRecomm: RDD[(Int, Seq[Rating])]): (Double, Double, Double) = {
    ...
    val (tp, tpFn, tpFp) = userTest.join(userRecomm).values.map { case (tests, recomms) =>
      ...
      ((t1._1 + t2._2), (t1._2 + t1._2), (t1._3 + t2._3))
    }
  ...
  def evaluatePopularity(trainingSet: RDD[Rating], userRecomm: RDD[(Int, Seq[Rating])]): Double = {
    val itemCoutns = trainingSet.map(_.product -> 1).reduceByKey(_ + _)
    val itemRecomm = userRecomm.values.flatMap(_.map(_.product -> 1))
    val (ret, n) = itemCoutns.join(itemRecomm).values.map { case (totalCount, count) => (math.log(1 + totalCount), count) }.reduce { (t1, t2) => (t1._1 + t2._1, t1._2 + t2._2)
    }
    ret.toDouble / n
  }
  ...
  def main1(args: Array[String]): Unit = {
    val parser = new scopt.OptionParser[Config]("recommendation") {
      ...
      opt[String]('a', "algorithm") required() action { (x, c) => c.copy(algorithm = x) } text ("algorithm name")
      opt[String]('s', "similarity method") action { (x, c) => c.copy(similarityMethod = x) } text ("similarity method")
      opt[String]('r', "recommend method") action { (x, c) => c.copy(recommendMethod = x) } text ("recommend method")
      opt[Int]('n', " number of recommendations") action { (x, c) => c.copy(numRecommendations = x) } text ("number of recommendations")
      opt[String]('o', "output-path") action { (x, c) => c.copy(outputPath = x) } text ("output path")
    ...
    val spark = SparkSession.builder().appName("Recommendation").master("local[*]").getOrCreate()
    ...
    // red logs - logs are pre-grouped by user-item
    ...
    val trainingSet = sc.union(ratingSplits.dropRight(1)).persist()
    val testingSet = ratingSplits.last.persist()
    ...
    val userRecomm = config.algorithm match {
      case "sim" =>
        ...
        SimilarityReconmender.recommend(trainingSet, params)
      case "als" =>
        val params = commonParams ++ Map(
          "recommendMethod" -> config.recommendMethod
        )
        AlsRecommender.recommend(trainingSet, params)
      case _ => throw new IllegalArgumentException("Unknown algorithm " + config.algorithm)
    }
    userRecomm.persist()
    ...
    logger.info("Precision=%.2f%% REcall=%.2f%% F1=%.4f Coverage=%.2f%% Popularity=%.4f".format(precision * 100, recall * 100, f1, coverage * 100, popularity))
